...
std::bitset<32> CellStatechart::GetState(){
  std::bitset<32> state;
...
void CellStatechart::SetState(std::bitset<32> state){
    if(state[1]==1){ 
...
sc::result Differentiation_Precursor::react(const EvDifferentiationUpdate &){
     CellPtr myCell=context<CellStatechart>().pCell;
    if (GetTime()>1 &&  GetDistanceFromDTC(myCell) > 200){
...
Differentiation_SpermFated::Differentiation_SpermFated(my_context ctx) :my_base(ctx){
    ...
    context<CellStatechart>().SpermDevelopmentDelay = 0.0;
};

sc::result Differentiation_SpermFated::react(const EvDifferentiationUpdate &){
    CellPtr myCell = context<CellStatechart>().pCell;
    context<CellStatechart>().SpermDevelopmentDelay += GetTimestep();
    if (context<CellStatechart>().SpermDevelopmentDelay > GlobalParameterStruct::Instance()->GetParameter(23)){
...
sc::result Differentiation_Sperm::react(const EvDifferentiationUpdate &){
    CellPtr myCell = context<CellStatechart>().pCell;
    if (context<CellStatechart>().SpermatocyteDivisions == 1){
        SetRadius(myCell, 1.5);
        SetReadyToDivide(myCell, true);
        context<CellStatechart>().SpermatocyteDivisions++;
    }
    if (context<CellStatechart>().SpermatocyteDivisions == 0){
...
EXPORT_TEMPLATE_CLASS1(StatechartCellCycleModel, CellStatechart)
